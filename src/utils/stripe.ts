...
import { OrganizationType as Organization } from "~/server/db/schema/auth-schema";
...
export async function createEmbededCheckoutSession({
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    mode: "subscription",
    return_url: `${env.BASE_URL}/finish-registration?session_id={CHECKOUT_SESSION_ID}`,
    customer: organization.stripeCustomerId || undefined,
    client_reference_id: user.id.toString(),
    allow_promotion_codes: true,
    subscription_data: {
...
export async function createCustomerPortalSession(organization: Organization) {
  ...
  if (configurations.data.length > 0 && configurations.data[0]) {
    ...
    if (!product.active) {
      throw new Error("Organization's subscription is not active in Stripe");
    }
    ...
    if (prices.data.length === 0) {
      throw new Error(
        "No active prices found for the Organization's subscription",
      );
    }
